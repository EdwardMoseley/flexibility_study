...
if [[ -n "$LIMIT" ]]; then
    "$PYTHON_BIN" build_csv_manifests.py "$CSV_PATH" "$OUTPUT_DIR" --limit "$LIMIT" --aggregate-manifest "$AGGREGATED_MANIFEST"
else
    "$PYTHON_BIN" build_csv_manifests.py "$CSV_PATH" "$OUTPUT_DIR" --aggregate-manifest "$AGGREGATED_MANIFEST"
fi
